#include "pi_hat_hardware_interface/pi_hat_hardware_interface.hpp"
...
namespace pi_hat_hardware_interface {
hardware_interface::CallbackReturn PiHatHardwareInterface::on_init(
...
std::vector<hardware_interface::StateInterface> PiHatHardwareInterface::export_state_interfaces() {
...
hardware_interface::CallbackReturn PiHatHardwareInterface::on_configure(
  ...
  RCLCPP_INFO(rclcpp::get_logger("PiHatHardwareInterface"), "Successfully configured!");

  return hardware_interface::CallbackReturn::SUCCESS;
}

std::vector<hardware_interface::CommandInterface>
PiHatHardwareInterface::export_command_interfaces() {
...
hardware_interface::CallbackReturn PiHatHardwareInterface::on_activate(
    const rclcpp_lifecycle::State & /*previous_state*/) {
  ...
  RCLCPP_INFO(rclcpp::get_logger("PiHatHardwareInterface"), "Successfully activated!");

  return hardware_interface::CallbackReturn::SUCCESS;
}

hardware_interface::CallbackReturn PiHatHardwareInterface::on_deactivate(
    const rclcpp_lifecycle::State & /*previous_state*/) {
  // Disable actuators
  spi_command_->flags[0] = 0;
  spi_command_->flags[1] = 0;
  ...
  RCLCPP_INFO(rclcpp::get_logger("PiHatHardwareInterface"), "Successfully deactivated!");

  return hardware_interface::CallbackReturn::SUCCESS;
}

hardware_interface::return_type PiHatHardwareInterface::read(const rclcpp::Time & /*time*/,
  ...
  // RCLCPP_INFO(rclcpp::get_logger("PiHatHardwareInterface"), "Joint 0: %f",
  // spi_data_->q_abad[1]);
  ...
  // Print the IMU
  // RCLCPP_INFO(rclcpp::get_logger("PiHatHardwareInterface"), "IMU: %f, %f, %f, %f, %f, %f, %f,
  // %f, %f, %f",
...
hardware_interface::return_type PiHatHardwareInterface::write(const rclcpp::Time & /*time*/,
                                                              const rclcpp::Duration & /*period*/) {
  copy_actuator_commands();
  return hardware_interface::return_type::OK;
}

void PiHatHardwareInterface::do_homing() {
  RCLCPP_INFO(rclcpp::get_logger("PiHatHardwareInterface"), "Homing actuators...");
    ...
    for (auto i = 0u; i < hw_state_positions_.size(); i++) {
      filtered_torques[i] =
          ...
          alpha * ((hw_command_positions_[i] - hw_state_positions_[i]) * hw_command_kps_[i] -
                   hw_state_velocities_[i] * hw_command_kds_[i]);
      ...
      if (hw_actuator_is_homed_[i]) {
        ...
        RCLCPP_INFO(rclcpp::get_logger("PiHatHardwareInterface"), "Homed actuator %d", i);
        all_homed = false;
      } else {
  ...
  RCLCPP_INFO(rclcpp::get_logger("PiHatHardwareInterface"), "Finished homing!");
}

void PiHatHardwareInterface::copy_actuator_commands() {
...
void PiHatHardwareInterface::copy_actuator_states() {
  // Iterate through the joints
  for (auto i = 0u; i < hw_state_positions_.size(); i++) {
...
PLUGINLIB_EXPORT_CLASS(pi_hat_hardware_interface::PiHatHardwareInterface,
