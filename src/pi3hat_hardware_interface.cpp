...
namespace pi3hat_hardware_interface
{
        ...
        const rclcpp::Time & /*time*/, const rclcpp::Duration &period)
    {
        ...
        // RCLCPP_INFO(rclcpp::get_logger("Pi3HatHardwareInterface"), "Joint 0: %f", spi_data_->q_abad[1]);
        ...
        // Print the IMU
        // RCLCPP_INFO(rclcpp::get_logger("Pi3HatHardwareInterface"), "IMU: %f, %f, %f, %f, %f, %f, %f, %f, %f, %f",
    ...
    void Pi3HatHardwareInterface::do_homing()
    {
        RCLCPP_INFO(rclcpp::get_logger("Pi3HatHardwareInterface"), "Homing actuators...");
        ...
        while (!all_homed)
        {
            ...
            for (auto i = 0u; i < hw_state_positions_.size(); i++)
            {
                ...
                else if (std::abs(filtered_torques[i]) >= hw_actuator_homing_torque_thresholds_[i])
                {
                    ...
                    RCLCPP_INFO(rclcpp::get_logger("Pi3HatHardwareInterface"), "Homed actuator %d", i);
                    all_homed = false;
                }
                else
                {
        ...
        RCLCPP_INFO(rclcpp::get_logger("Pi3HatHardwareInterface"), "Finished homing!");
    }

    void Pi3HatHardwareInterface::copy_actuator_commands()
    {
    ...
    void Pi3HatHardwareInterface::copy_actuator_states()
    {
        // Iterate through the joints
        for (auto i = 0u; i < hw_state_positions_.size(); i++)
        {
